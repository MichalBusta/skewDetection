...
public:
	...
	virtual double detectSkew( std::vector<cv::Point>& outerContour, bool approximate = false, cv::Mat* debugImage = NULL );
	...
	void voteInHistogram( std::vector<cv::Point>& contourOrig, double *histogram, double weight, bool approximate = false, cv::Mat* debugImage = NULL);
